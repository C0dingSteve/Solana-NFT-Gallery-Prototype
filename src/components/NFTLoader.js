...
import { getParsedNftAccountsByOwner, isValidSolanaAddress, createConnectionConfig} from "@nfteyez/sol-rayz";
import axios from "axios";
import ModelLoader from "./GLTFViewer";
...
export default function NFTLoader({phantomState}) {
    ...
    const getNftTokenData = async () => {
        try {
            ...
            for (let i = 0; i < n; i++) {
                ...
                //let val = await axios.get(valprev.image);
                console.log("ggg");
                console.log(data[i].data.uri + " f555");
                arr.push(val);
                Mod.push(val.data.properties.files[0])
            }
            nftData.map((val, ind) => {
                console.log(val);
                console.log(data);
    ...
    return (
    <section className='nft mt-2 my-5' id="can">
        {phantomState?(
            <div className="container text-center">
                <div className="row">
                {loading?(
                    <>
                        ...
                        {nftData && nftData.length > 0 && nftData.map((val, ind) => {
                            if(val.data.properties.category === 'image')
                                return(
                                    <div className="col-4 mt-3" key={ind}>
                                        <img style={{width:'50px'}} className='w-100' src={val.data.image} alt="loading..." />
                                        <p className="mt-1">{val.data.name}</p>
                                        <h6 className=" mt-2">
                                            {val.data.symbol}
                                        </h6>
                            ...
                            else 
                            {
                                return(
                                    <div className="col-4 mt-3" key={ind}>
                                        <GLTFViewer url={val.data.properties.files[0].uri}/>
                                        <p className="mt-1"> {val.data.name} </p>
                                        <h6 className=" mt-2"> 
                                            {val.data.symbol}
                                        </h6>
